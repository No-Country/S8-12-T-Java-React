...
export default function Login() {
  ...
  function FormError() {
    if (emailValue === "" || inputError == "Error: Username not found") {
      ...
      setEmailResult({
        color: "#f02849",
        ErrorText: "El correo es incorrecto.",
      });
    } else {
      setEmailResult({
        color: "",
    ...
    if (passwordValue === "" || inputError === "Error: Invalid password") {
      ...
      setPasswordResult({
        color: "#f02849",
        ErrorText: "La contraseña incorrecta.",
      });
    } else {
      event.preventDefault();
      Login();
  ...
  return (
    <>
      <div className="sm:w-full sm:h-[80vh] flex flex-col sm:justify-center sm:items-center sm:bg-white">
